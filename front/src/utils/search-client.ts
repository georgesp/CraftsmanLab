...
export function searchAll(query: string): SearchHit[] {
  ...
  matched.sort((a, b) => {
    ...
    const aInKeywords = a.searchKeywords?.[currentLang]?.some(k => k.toLowerCase().includes(q)) || false;
    const bInKeywords = b.searchKeywords?.[currentLang]?.some(k => k.toLowerCase().includes(q)) || false;
